#!/usr/bin/env python3
"""Signpost複数要員紹介テンプレートのFocused Test。"""
...
DECISION = (
    "multi_item_mail",
    "signpost_multi_resource_intro_v1",
)


class SignpostMultiResourceIntroTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_real_two_are_excluded_and_preserved(self):
    ...
    def test_message_id_url_variable_and_two_three_profiles(self):
        expected_profiles = {
            "1a0d1454442c23cf": 3,
            "1a0d143ae017c9d5": 2,
        }
        for mid, expected in expected_profiles.items():
            ...
            sequence = iter(range(1, expected + 1))
            changed["body_text"] = re.sub(
                r"(?m)^(スキルシート[ \t]*[:：][ \t]*)https?://\S+",
                lambda match: "{}https://skills.example.invalid/person/{}".format(
                    match.group(1), next(sequence)
                ),
                changed["body_text"],
            )
            ...
            with self.subTest(message_id=mid):
                self.assertEqual(len(target._SIGNPOST_NAME_RE.findall(body)), expected)
                self.assertEqual(
                    len(target._SIGNPOST_SKILL_SHEET_URL_RE.findall(body)), expected
                )
    ...
    def test_same_sender_single_resource_emails_are_kept(self):
        ...
        same_sender_ids = {
            ...
            == "sales@signpost-g.co.jp"
        }
        single_ids = same_sender_ids - TARGET_IDS
        self.assertEqual(len(single_ids), 3)
        for mid in single_ids:
            with self.subTest(message_id=mid):
                self.assertIsNone(target.detect_template_exclusion(self.master[mid]))

        representative = self.master["1a0d14bf5b8418e5"]
        body = target.unicodedata.normalize(
            "NFKC", representative["body_text"]
        ).replace("\r\n", "\n").strip()
        self.assertEqual(len(target._SIGNPOST_NAME_RE.findall(body)), 1)
        self.assertEqual(len(target._SIGNPOST_SKILL_SHEET_URL_RE.findall(body)), 1)
    ...
    def test_url_count_mismatch_and_other_sender_are_kept(self):
        for mid in TARGET_IDS:
            ...
            mismatched["body_text"] = re.sub(
                r"(?m)^スキルシート[ \t]*[:：][ \t]*https?://\S+[ \t]*$",
                "",
                mismatched["body_text"],
                count=1,
            )
            with self.subTest(message_id=mid, guard="url_count"):
    ...
    def test_latest_input_memory_replay_and_confirm(self):
        ...
        self.assertEqual(len(resource_ids - TARGET_IDS), 1833)
