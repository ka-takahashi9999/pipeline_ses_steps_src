#!/usr/bin/env python3
"""Aliplaza複数要員紹介テンプレートのFocused Test。"""
...
TARGET_ID = "1a0d126231435729"
EXISTING_THREE_ID = "1a0ad4dad3d31df2"
...
DECISION = (
    "multi_item_mail",
    "aliplaza_multi_resource_intro_v1",
)
EXISTING_THREE_DECISION = (
    "multi_item_mail",
    "aliplaza_three_resource_blocks_v1",
)


class AliplazaMultiResourceIntroTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        ...
        archived_path = (
            STEP / "02_confirm" / "focused_template_exclusion" / "01_result"
            / "99_removed_individual_emails_detail.jsonl"
        )
        cls.archived = read_jsonl_as_dict(str(archived_path))
    ...
    def _fixture(self, profile_count):
        ...
        fixture["subject"] = (
            "【09/30】【JAVA SE {}名】【要件定義~】Java/Spring Boot".format(
                profile_count
            )
        )
        intro = """株式会社テクノヴァース
...
早速ですが、 弊社現在注力中の要員情報を共有いたします。
見合う案件がございましたら、
ぜひご紹介いただけますと幸いです。
"""
        blocks = []
        for number in range(1, profile_count + 1):
            blocks.append("""要員情報{number}
■所属：弊社社員
■経験：5年
■日本語：N1
■スキル：Java
...
""".format(number=number))
        ...
        fixture["attachments"] = [
            {
                "filename": "可変_要員{}_スキル経歴書.xlsx".format(number),
    ...
    def test_real_four_is_excluded_and_preserved(self):
    ...
    def test_existing_three_keeps_existing_rule_id(self):
        existing = self.archived[EXISTING_THREE_ID]
        self.assertEqual(
            target.detect_template_exclusion(existing), EXISTING_THREE_DECISION
        )

        generalized = copy.deepcopy(existing)
        generalized["message_id"] = "future-generalized-three"
        generalized["subject"] = generalized["subject"].replace("SE3名", "SE 3名")
        self.assertEqual(target.detect_template_exclusion(generalized), DECISION)
    ...
    def test_single_resource_emails_are_kept(self):
        ...
        saved_single = self.master["1a0d287381fc43c6"]
    ...
    def test_attachment_mismatch_and_other_sender_are_kept(self):
        ...
        other_sender["from"] = "Other Company <feng-h@example.com>"
