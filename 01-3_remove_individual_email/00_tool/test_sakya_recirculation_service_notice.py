#!/usr/bin/env python3
"""Sakya失注要員・再募集案件受付サービス案内のFocused Test。"""
...
TARGET_ID = "1a0d0f14e9ad61c8"
SAKYA_PROCUREMENT_ID = "1a0d1dc366b84837"
...
DECISION = (
    ...
    "sakya_recirculation_service_notice_v1",
)
...
class SakyaRecirculationServiceNoticeTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_message_id_is_variable(self):
        ...
        changed["message_id"] = "future-recirculation-service-notice"
    ...
    def test_same_sender_normal_mails_are_kept(self):
        same_sender_ids = {
            ...
            if target.extract_email(record.get("from") or "") == "koki.odake@sakya.jp"
        }
        normal_ids = same_sender_ids - {TARGET_ID}
        self.assertEqual((len(same_sender_ids), len(normal_ids)), (38, 37))
        for mid in normal_ids:
            with self.subTest(message_id=mid):
                self.assertIsNone(target.detect_template_exclusion(self.master[mid]))

    def test_all_sakya_mails_and_procurement_rule_do_not_overlap(self):
        sakya_ids = {
            mid for mid, record in self.master.items()
    ...
    def test_missing_conditions_are_kept(self):
        changes = (
            ...
            (
                "attachment",
                "attachments",
                [{"filename": "individual.xlsx", "mime_type": "application/vnd.ms-excel"}],
            ),
            (
                "individual_profile",
                "body_text",
                self.original["body_text"] + "\n【氏名】個別要員\n【単価】70万円",
            ),
        )
    ...
    def test_latest_input_memory_replay_and_confirm(self):
        ...
        expected_new_ids = PREVIOUS_RESOURCE_IDS | {TARGET_ID, SAKYA_PROCUREMENT_ID}
        ...
        self.assertEqual(len(project_ids - {SAKYA_PROCUREMENT_ID}), 810)
        self.assertEqual(
