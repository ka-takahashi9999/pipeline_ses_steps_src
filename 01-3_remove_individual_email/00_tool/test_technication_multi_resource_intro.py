#!/usr/bin/env python3
"""Technication複数要員紹介テンプレートのFocused Test。"""
...
EXISTING_SUBJECT_EXCLUDED_ID = "1a0d15d021ea8417"
DECISION = (
    "multi_item_mail",
    "technication_multi_resource_intro_v1",
)


class TechnicationMultiResourceIntroTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_message_id_independent_and_two_three_profiles(self):
        expected_profiles = {
            "1a0d15e5f06de61a": 3,
            "1a0d14c378c2ff81": 2,
            "1a0d12c92e945be9": 2,
        }
        for mid, expected in expected_profiles.items():
            changed = copy.deepcopy(self.master[mid])
            changed["message_id"] = "future-{}".format(mid[-4:])
            body = target.unicodedata.normalize(
                "NFKC", changed["body_text"]
            ).replace("\r\n", "\n").strip()
            excel_count = sum(
                Path(attachment["filename"]).suffix.lower()
                in target._EXCEL_SKILL_SHEET_SUFFIXES
                for attachment in changed["attachments"]
            )
            with self.subTest(message_id=mid):
                self.assertEqual(len(target._TECHNICATION_NAME_RE.findall(body)), expected)
                self.assertEqual(excel_count, expected)
    ...
    def test_single_resource_emails_are_kept(self):
        prev_ids = {row["message_id"] for row in self.prev}
        technication_ids = {
            mid for mid, record in self.master.items()
            if mid in prev_ids
            and target.extract_email(record.get("from") or "").endswith("@technication.co.jp")
        }
        project_ids = {
            mid for mid in technication_ids
            if self.master[mid].get("subject", "").startswith("【案件紹介")
        }
        single_ids = (
            technication_ids - TARGET_IDS - project_ids - {EXISTING_SUBJECT_EXCLUDED_ID}
        )
        self.assertEqual(len(single_ids), 11)
        for mid in single_ids:
            with self.subTest(message_id=mid):
                self.assertIsNone(target.detect_template_exclusion(self.master[mid]))

        representative = self.master["1a0d166b78d83883"]
        self.assertEqual(len(representative["attachments"]), 1)
        self.assertIsNone(target.detect_template_exclusion(representative))

    def test_attachment_mismatch_other_sender_and_existing_p1(self):
        for mid in TARGET_IDS:
            mismatched = copy.deepcopy(self.master[mid])
            mismatched["attachments"] = mismatched["attachments"][:-1]
            with self.subTest(message_id=mid, guard="attachment_count"):
                self.assertIsNone(target.detect_template_exclusion(mismatched))

        other_sender = copy.deepcopy(self.master[sorted(TARGET_IDS)[0]])
        other_sender["from"] = "Other Company <resource@example.com>"
        self.assertIsNone(target.detect_template_exclusion(other_sender))

        existing = self.master[EXISTING_SUBJECT_EXCLUDED_ID]
        self.assertIsNone(target.detect_template_exclusion(existing))
        self.assertEqual(
            target.determine_exclusion_reason(existing, *self.rules),
            "multiple_resource_explicit_subject_count",
        )
    ...
    def test_latest_input_memory_replay_and_confirm(self):
        ...
        expected_resource_ids = PREVIOUS_IMPLEMENTED_IDS | TARGET_IDS
        ...
        self.assertEqual(detected_ids, TARGET_IDS)

        spec = importlib.util.spec_from_file_location(
            "confirm_technication",
