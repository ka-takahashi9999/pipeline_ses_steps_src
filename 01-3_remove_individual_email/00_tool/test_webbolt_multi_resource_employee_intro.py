#!/usr/bin/env python3
"""Webbolt複数要員紹介テンプレートのFocused Test。"""
...
DECISION = (
    "multi_item_mail",
    "webbolt_multi_resource_employee_intro_v1",
)


class WebboltMultiResourceEmployeeIntroTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_message_id_and_subject_skills_are_variable(self):
        changed = copy.deepcopy(self.original)
        changed["message_id"] = "future-message-id"
        changed["subject"] = "◎◎弊社社員（Go /Rust /Python）の紹介です。"
    ...
    def test_single_employee_is_kept(self):
        single = copy.deepcopy(self.original)
        single["message_id"] = "single-employee-fixture"
        single["body_text"] = """（BCCでご案内しております。）
お世話になります。ウェブボルトの増山です。
標題の件、弊社要員をご紹介させていただきます。
見合う案件がございましたら、SINGLE-01
ご紹介くださいますよう、よろしくお願い申しあげます。
-----------------------------------------------------------------
【名 前】AA：30才（男性）
【所 属】弊社契約社員
【最寄駅】新宿駅
...
【単 金】60万円
"""
        single["attachments"] = [copy.deepcopy(self.original["attachments"][0])]
        self.assertIsNone(target.detect_template_exclusion(single))
        self.assertIsNone(target.determine_exclusion_reason(single, set(), []))

    def test_attachment_count_mismatch_and_other_sender_are_kept(self):
        mismatched = copy.deepcopy(self.original)
    ...
    def test_downstream_existing_artifact_id_reconciliation(self):
        paths = {
            ...
            "06-1": (
                ROOT / "06-1_match_budget" / "01_result" / "matched_pairs_budget.jsonl",
                362,
                lambda row: (row.get("resource_info") or {}).get("message_id"),
            ),
            "06-8": (
                ROOT / "06-8_match_skill_category" / "01_result"
                / "matched_pairs_skill_category.jsonl",
                37,
                lambda row: (row.get("resource_info") or {}).get("message_id"),
            ),
            "06-12": (
