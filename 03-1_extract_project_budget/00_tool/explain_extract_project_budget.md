# explain_extract_project_budget
...
## 出力ファイルと構造
- `extract_project_budget.jsonl`
- `99_price_null_extract_project_budget.jsonl`
- 主な辞書/レコードキー: `message_id`, `unit_price`, `unit_price_sub_infor`

## 処理ロジックの詳細
- `_n`: NFKC正規化（全角→半角、波ダッシュ等を統一）
- `_get_segments`: 価格キーワードを含む行と、その直後の最小価格値行だけを返す
- `_monthly_explicit`: /月 明示付きパターン（日給より優先）
- `_hourly_daily`: 時給・日給を検出して月換算で返す
- `_man_patterns`: 万円パターンで価格を抽出
- `rule_extract`: ルールベースで単価を抽出。
