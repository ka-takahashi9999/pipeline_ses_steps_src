...
class LinkBundleAdapter:
    ...
    def _new_source(self, mail: Dict[str, Any], links_value: Any) -> Source:
        snapshot_complete = mail.get("html_links_snapshot_complete", True) is True
        ...
        return Source(
            ...
            configured_cross_check_authorities=[
                ...
                CardinalityAuthority.SNAPSHOT_SET.value,
            ],
        )
    ...
    def _parse(self, mail: Dict[str, Any]) -> ParseResult:
        ...
        source = self._new_source(mail, links_value)
        ...
        source.cardinality_evidence = [
            ...
            CardinalityEvidence(
                authority=CardinalityAuthority.SNAPSHOT_SET.value,
                source="SAVED_HTML_LINKS_SNAPSHOT",
                count=len(candidates),
                complete=source.acquisition_status == "COMPLETE" and structural_complete,
                reasons=([] if source.acquisition_status == "COMPLETE" else ["snapshot_incomplete"])
                + structural_reasons,
            ),
        ]
        gate = evaluate_completeness(source, containers, candidates)
        source_dict = self._source_dict(source, classified, candidates, links)
        ...
        items = self._build_items(source, classified, candidates)
        if len({item["logical_item_id"] for item in items}) != len(items):
            ...
            return ParseResult(
                ...
                gate.reasons,
                [],
                self._source_dict(source, classified, candidates, links),
                [container.to_dict() for container in containers],
                [row.to_dict() for row in classified],
            )
        return ParseResult(
