...
09-4 で category mismatch を除外した営業候補から営業メールドラフトと preview を生成する。
"""
...
INPUT_BASE_DIR = project_root / "09-4_remove_category_mismatch_sales_candidates/01_result"
INPUT_FILE_TEMPLATE = "sales_proposal_candidates_{date}.jsonl"
...
def base_pair_file_stem(pair_file_name: Any) -> str:
    ...
    if stem.endswith(PREVIOUS_OUTPUT_SUFFIX):
        return stem[: -len(PREVIOUS_OUTPUT_SUFFIX)]
    return stem
...
def find_latest_input() -> Tuple[Path, str]:
    candidates = sorted(INPUT_BASE_DIR.glob("sales_proposal_candidates_*.jsonl"))
    if not candidates:
        raise FileNotFoundError(f"09-4入力JSONLが存在しません: {INPUT_BASE_DIR}")
    latest = candidates[-1]
    date_part = latest.stem.replace("sales_proposal_candidates_", "", 1)
...
def resolve_input(target_date: Optional[str]) -> Tuple[Path, str, str]:
    if target_date:
        input_path = INPUT_BASE_DIR / INPUT_FILE_TEMPLATE.format(date=target_date)
        if not input_path.exists():
            raise FileNotFoundError(f"対象日付の09-4入力JSONLが存在しません: {input_path}")
...
def generate_record(record: Dict[str, Any], mail_mode: str) -> Tuple[Dict[str, Any], str, str, str, str]:
    ...
    preview_file_name = build_preview_file_name(record, mail_mode)
    note_file_name = build_preview_file_name(record, mail_mode, is_note=True)
...
def main() -> None:
    ...
    try:
        ...
        logger.info(f"09-4 input path: {input_path}")
